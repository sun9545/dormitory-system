...
// const char* WIFI_PASSWORD = "YOUR_WIFI_PASSWORD"; // 已删除硬编码
 ...
 const char* DEVICE_ID = "FP001-9-6";
...
// WiFi界面事件回调函数声明
 ...
 void createOptimizedFunctionButtons() {
    ...
    // ⭐⭐⭐ 第三行：备份、恢复（2个按钮居中）
    int row3Y = row2Y + btnHeight + spacing;
    int startX3 = (320 - (2 * btnWidth + spacing)) / 2;  // 2个按钮居中
    
    // 备份按钮
    lv_obj_t *btnBackup = lv_btn_create(mainScreen);
    lv_obj_set_size(btnBackup, btnWidth, btnHeight);
    lv_obj_set_pos(btnBackup, startX3, row3Y);
    lv_obj_set_style_bg_color(btnBackup, lv_color_hex(0x607D8B), 0);  // 灰蓝色
    lv_obj_set_style_radius(btnBackup, 8, 0);
    lv_obj_add_event_cb(btnBackup, btnBackup_event_cb, LV_EVENT_CLICKED, NULL);
    
    lv_obj_t *backupLabelBtn = lv_label_create(btnBackup);
    lv_label_set_text(backupLabelBtn, "备份");
    lv_obj_set_style_text_font(backupLabelBtn, &myFont_new, 0);
    lv_obj_set_style_text_color(backupLabelBtn, lv_color_hex(0xFFFFFF), 0);
    lv_obj_center(backupLabelBtn);
    
    // 恢复按钮
    lv_obj_t *btnRestore = lv_btn_create(mainScreen);
    lv_obj_set_size(btnRestore, btnWidth, btnHeight);
    lv_obj_set_pos(btnRestore, startX3 + btnWidth + spacing, row3Y);
    lv_obj_set_style_bg_color(btnRestore, lv_color_hex(0x795548), 0);  // 棕色
    lv_obj_set_style_radius(btnRestore, 8, 0);
    lv_obj_add_event_cb(btnRestore, btnRestore_event_cb, LV_EVENT_CLICKED, NULL);
    
    lv_obj_t *restoreLabelBtn = lv_label_create(btnRestore);
    lv_label_set_text(restoreLabelBtn, "恢复");
    lv_obj_set_style_text_font(restoreLabelBtn, &myFont_new, 0);
    lv_obj_set_style_text_color(restoreLabelBtn, lv_color_hex(0xFFFFFF), 0);
    lv_obj_center(restoreLabelBtn);
}
...
void btnHeartbeat_event_cb(lv_event_t * e) {
 ...
 void btnStats_event_cb(lv_event_t * e) {
     ...
     if (statisticsInProgress) {
         Serial.println("WARN 统计数据获取正在进行中,请等待...");
         return;
     }
     
     // 1. 立即显示"正在获取数据"的提示框
     showMessageBox("统计数据", "正在获取统计数据...\n请稍候", "获取中", true);
     
     // 2. 设置统计获取状态
     statisticsInProgress = true;
     
     // 3. 创建异步定时器,500ms后执行实际的数据获取
     // 这样UI可以立即响应,用户能看到弹窗
     lv_timer_t *fetchTimer = lv_timer_create(statistics_fetch_timer_cb, 500, NULL);
     lv_timer_set_repeat_count(fetchTimer, 1);
     
     Serial.println("STATS 统计数据获取已启动,异步执行中...");
 }
...
void checkAndSyncTime() {
 ...
 void statistics_fetch_timer_cb(lv_timer_t * timer) {
     ...
     if (stats.success) {
         ...
         updateMsgBox("统计数据", errorMsg, "确定", false);
...
struct FingerprintTemplate {
    int id;
    uint8_t model[512];  // 最大512字节（实际可能更小）
    uint16_t actualSize; // ⭐ 实际模板大小（通常256-384字节）
    bool valid;
};

// ⭐ 动态指针和计数（完全自适应，不写死数量）
FingerprintTemplate *fingerprintBackup = nullptr;
int backupCount = 0;
int backupCapacity = 0;

// 进度回调函数类型
typedef void (*ProgressCallback)(int current, int total, String status);

// 进度界面全局变量
static lv_obj_t *migrationProgressScreen = NULL;
static lv_obj_t *migrationTitleLabel = NULL;
static lv_obj_t *migrationProgressLabel = NULL;
static lv_obj_t *migrationStatusLabel = NULL;
static lv_obj_t *migrationBar = NULL;

/**
 * 第一步：扫描并统计实际指纹数量
 */
int scanFingerprintCount() {
    Serial.println("==================== 第1步：扫描指纹数量 ====================");
    ...
    Serial.println();
    Serial.println("✅ 扫描完成，发现 " + String(count) + " 个指纹");
    Serial.println("=================================================");
 ...
 * 第二步：根据实际数量动态分配PSRAM
 */
bool allocateBackupBuffer(int count) {
    // 先释放旧的
    if (fingerprintBackup != nullptr) {
        Serial.println("⚠️ 释放旧的备份缓冲区");
        heap_caps_free(fingerprintBackup);
        fingerprintBackup = nullptr;
    }
    
    if (count == 0) {
        Serial.println("❌ 数量为0，无需分配");
        return false;
    }
    
    // 计算需要的内存
    size_t bufferSize = sizeof(FingerprintTemplate) * count;
    Serial.println("==================== 第2步：分配PSRAM缓冲区 ====================");
    Serial.printf("实际需要: %d 个指纹\n", count);
    Serial.printf("内存大小: %.2f KB (%d 字节)\n", bufferSize / 1024.0, bufferSize);
    
    // 检查PSRAM可用空间
    size_t freePsram = ESP.getFreePsram();
    Serial.printf("可用PSRAM: %.2f MB\n", freePsram / 1024.0 / 1024.0);
    
    if (bufferSize > freePsram) {
        Serial.println("❌ PSRAM空间不足！");
        return false;
    }
    
    // ⭐ 从PSRAM动态分配（精确大小）
    fingerprintBackup = (FingerprintTemplate*)heap_caps_malloc(
        bufferSize, 
        MALLOC_CAP_SPIRAM
    );
    ...
    for (int i = 0; i < count; i++) {
        fingerprintBackup[i].valid = false;
    }
    
    backupCapacity = count;
    backupCount = 0;
    
    Serial.println("✅ PSRAM缓冲区分配成功");
    Serial.printf("缓冲区地址: 0x%08X\n", (uint32_t)fingerprintBackup);
    Serial.printf("剩余PSRAM: %.2f MB\n", ESP.getFreePsram() / 1024.0 / 1024.0);
    Serial.println("=================================================");
 ...
 * 释放PSRAM备份缓冲区
 */
void freeBackupBuffer() {
    if (fingerprintBackup != nullptr) {
        size_t freedSize = backupCapacity * sizeof(FingerprintTemplate);
        Serial.println("🧹 释放PSRAM备份缓冲区");
        Serial.printf("释放内存: %.2f KB\n", freedSize / 1024.0);
        ...
        Serial.printf("释放后PSRAM: %.2f MB\n", ESP.getFreePsram() / 1024.0 / 1024.0);
    }
}

/**
 * 从R307读取模板数据到缓冲区（返回实际读取的字节数）
 * @param buffer 缓冲区指针
 * @param bufferSize 缓冲区最大大小
 * @param actualSize 输出参数：实际读取的字节数
 * @return true=成功, false=失败
 */
bool readTemplateFromSensor(uint8_t *buffer, uint16_t bufferSize, uint16_t *actualSize) {
    *actualSize = 0;  // 初始化
    ...
    // 发送UpChar命令（0x08），参数：缓冲区号（0x01 = CharBuffer1）
    uint8_t cmd[] = {0xEF, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, 0x01, 0x00, 0x04, 0x08, 0x01, 0x00, 0x0E};
    fingerprintSerial.write(cmd, 13);
    fingerprintSerial.flush();
    
    delay(300);  // 等待传感器处理
    
    // 读取确认包
    if (fingerprintSerial.available() < 12) {
        Serial.println("⚠️  UpChar无响应，available: " + String(fingerprintSerial.available()));
        return false;
    }
    
    // 跳过包头和地址 (6字节) + 包标识(1) + 长度(2)
    for (int i = 0; i < 9; i++) fingerprintSerial.read();
    
    uint8_t confirmCode = fingerprintSerial.read();
    if (confirmCode != 0x00) {
        Serial.println("❌ UpChar失败，确认码: 0x" + String(confirmCode, HEX));
        return false;
    }
    ...
    Serial.println("📥 开始读取模板数据包...");
    
    while (bytesRead < bufferSize && (millis() - startTime < 8000)) {
        // 等待足够的数据
        int waitCount = 0;
        while (fingerprintSerial.available() < 11 && waitCount < 100) {
            delay(10);
            waitCount++;
        }
        
        if (fingerprintSerial.available() >= 11) {
            // 读取包头
            if (fingerprintSerial.read() != 0xEF) {
                continue;
            }
            if (fingerprintSerial.read() != 0x01) {
                continue;
            }
            ...
            // 读取长度
            uint16_t len = (fingerprintSerial.read() << 8) | fingerprintSerial.read();
            len -= 2;  // 减去校验和长度
            
            Serial.println("  包" + String(packetsReceived + 1) + ": 类型=0x" + String(packetId, HEX) + ", 长度=" + String(len) + " 字节");
            
            // 等待数据完全到达
            waitCount = 0;
            while (fingerprintSerial.available() < len + 2 && waitCount < 50) {
                delay(10);
                waitCount++;
            }
            
            // 读取数据
            for (uint16_t i = 0; i < len && bytesRead < bufferSize; i++) {
                if (fingerprintSerial.available()) {
                    buffer[bytesRead++] = fingerprintSerial.read();
                } else {
                    Serial.println("⚠️  数据包读取不完整");
                    break;
                }
            }
            
            // 读取校验和（丢弃）
            if (fingerprintSerial.available() >= 2) {
                fingerprintSerial.read();
                fingerprintSerial.read();
            }
            ...
            // 如果是结束包 (0x08) 或者已经读够了数据，退出
            if (packetId == 0x08) {
                *actualSize = bytesRead;
                Serial.println("✅ 模板读取完成 (结束包): " + String(bytesRead) + " 字节 (" + String(packetsReceived) + " 个包)");
                return true;
            }
            
            // ⭐ 修复：如果读够了数据（通常3-4个包就够了），也视为成功
            if (bytesRead >= 256 && packetsReceived >= 3) {
                *actualSize = bytesRead;
                Serial.println("✅ 模板读取完成 (数据包): " + String(bytesRead) + " 字节 (" + String(packetsReceived) + " 个包)");
                return true;
            }
        }
    }
    ...
    if (bytesRead >= 256) {
        *actualSize = bytesRead;
        Serial.println("✅ 模板读取完成 (超时但数据完整): " + String(bytesRead) + " 字节 (" + String(packetsReceived) + " 包)");
        return true;
    }
    
    Serial.println("⚠️  读取失败: 只有 " + String(bytesRead) + " 字节 (" + String(packetsReceived) + " 包)");
    return false;
}
 ...
 * 将模板数据写入R307
 */
bool writeTemplateToSensor(uint8_t *buffer, uint16_t bufferSize) {
    // 发送DownChar命令（0x09），参数：缓冲区号（0x01 = CharBuffer1）
    ...
    if (confirmCode != 0x00) {
        Serial.println("DownChar失败，确认码: " + String(confirmCode, HEX));
        return false;
    }
    ...
    while (bytesSent < bufferSize) {
        uint16_t remaining = bufferSize - bytesSent;
        uint16_t currentSize = (remaining < packetSize) ? remaining : packetSize;
        bool isLast = (bytesSent + currentSize >= bufferSize);
        
        // 包标识：0x02=数据包，0x08=结束包
        uint8_t packetId = isLast ? 0x08 : 0x02;
        uint16_t len = currentSize + 2;  // 数据 + 校验和
        
        // 计算校验和
        uint16_t checksum = packetId + (len >> 8) + (len & 0xFF);
        for (uint16_t i = 0; i < currentSize; i++) {
            checksum += buffer[bytesSent + i];
        }
        
        // 发送包头
        fingerprintSerial.write(0xEF);
        fingerprintSerial.write(0x01);
        fingerprintSerial.write(0xFF);
        fingerprintSerial.write(0xFF);
        fingerprintSerial.write(0xFF);
        fingerprintSerial.write(0xFF);
        fingerprintSerial.write(packetId);
        fingerprintSerial.write((len >> 8) & 0xFF);
        fingerprintSerial.write(len & 0xFF);
        
        // 发送数据
        fingerprintSerial.write(&buffer[bytesSent], currentSize);
        
        // 发送校验和
        fingerprintSerial.write((checksum >> 8) & 0xFF);
        fingerprintSerial.write(checksum & 0xFF);
        fingerprintSerial.flush();
    ...
    Serial.println("写入了 " + String(bytesSent) + " 字节模板数据");
    return true;
}
 ...
 * 备份单个指纹模板（记录实际大小）
 */
bool backupSingleFingerprint(int id) {
    if (fingerprintBackup == nullptr) {
        Serial.println("❌ 备份缓冲区未初始化");
        return false;
    }
    
    if (backupCount >= backupCapacity) {
        Serial.println("❌ 备份缓冲区已满");
        return false;
    }
    
    // 检查ID是否已录入，同时加载到CharBuffer1
    uint8_t p = finger.loadModel(id);
    if (p != FINGERPRINT_OK) {
        return false;  // 此ID没有指纹，跳过
    }
    
    Serial.println("📥 备份指纹ID: " + String(id));
    
    // 从传感器读取模板数据，并记录实际大小
    uint16_t actualSize = 0;
    if (!readTemplateFromSensor(fingerprintBackup[backupCount].model, 512, &actualSize)) {
        Serial.println("❌ 读取模板数据失败");
        return false;
    }
    
    // 保存到PSRAM
    fingerprintBackup[backupCount].id = id;
    fingerprintBackup[backupCount].actualSize = actualSize;  // ⭐ 记录实际大小
    fingerprintBackup[backupCount].valid = true;
    backupCount++;
    
    Serial.println("✅ ID " + String(id) + " 备份成功 (" + String(backupCount) + "/" + String(backupCapacity) + "), 大小: " + String(actualSize) + " 字节");
 ...
 * 第三步：备份所有指纹（智能自适应版本）
 */
int backupAllFingerprints(ProgressCallback progressCallback = nullptr) {
    Serial.println("==================== 开始智能备份流程 ====================");
    unsigned long startTime = millis();
    
    // ⭐ 第1步：扫描统计数量
    if (progressCallback != nullptr) {
        progressCallback(0, 100, "第1步: 扫描指纹数量");
    }
    
    int totalCount = scanFingerprintCount();
    
    if (totalCount == 0) {
        Serial.println("❌ 未发现任何指纹");
        return 0;
    }
    
    // ⭐ 第2步：精确分配内存
    if (progressCallback != nullptr) {
        progressCallback(10, 100, "第2步: 分配内存 " + String(totalCount) + " 个");
    }
    
    if (!allocateBackupBuffer(totalCount)) {
    ...
    // ⭐ 第3步：实际备份数据
    if (progressCallback != nullptr) {
        progressCallback(20, 100, "第3步: 备份数据...");
    }
    ...
    for (int id = 0; id < 1000; id++) {
        // 更新进度（20% - 100%，共80%给备份过程）
        if (progressCallback != nullptr && id % 10 == 0) {
            int percent = 20 + (id * 80 / 1000);
            String status = "备份中: " + String(successCount) + "/" + String(totalCount);
            progressCallback(percent, 100, status);
        }
        
        if (backupSingleFingerprint(id)) {
            successCount++;
        }
    ...
    Serial.println("==================== 备份完成 ====================");
    Serial.println("✅ 成功备份: " + String(successCount) + " 个指纹");
    Serial.println("⏱️  总耗时: " + String(duration) + " 秒");
    Serial.println("💾 内存占用: " + String((successCount * sizeof(FingerprintTemplate)) / 1024.0, 2) + " KB");
    Serial.println("📊 使用率: " + String(successCount * 100.0 / 1000, 1) + "% (传感器容量)");
    Serial.println("=================================================");
 ...
 * 恢复单个指纹（使用实际大小，带重试机制）
 */
bool restoreSingleFingerprint(int index, int maxRetries = 3) {
    if (fingerprintBackup == nullptr) {
        Serial.println("❌ 备份缓冲区未初始化");
        return false;
    }
    
    if (index >= backupCount || !fingerprintBackup[index].valid) {
        Serial.println("❌ 无效索引: " + String(index));
        return false;
    }
    
    int id = fingerprintBackup[index].id;
    uint16_t actualSize = fingerprintBackup[index].actualSize;  // ⭐ 使用实际大小
    
    Serial.println("📤 恢复指纹ID: " + String(id) + ", 大小: " + String(actualSize) + " 字节");
    
    // ⭐ 添加重试机制
    for (int retry = 0; retry < maxRetries; retry++) {
        if (retry > 0) {
            Serial.println("🔄 ID " + String(id) + " 重试第 " + String(retry) + " 次...");
            delay(100);  // 重试前等待
        }
        
        // ⭐ 使用实际大小写入，而不是固定的512字节
        if (!writeTemplateToSensor(fingerprintBackup[index].model, actualSize)) {
            Serial.println("❌ ID " + String(id) + " 写入模板失败 (尝试 " + String(retry + 1) + "/" + String(maxRetries) + ")");
        ...
        if (p != FINGERPRINT_OK) {
            Serial.println("❌ ID " + String(id) + " 存储失败: 0x" + String(p, HEX) + " (尝试 " + String(retry + 1) + "/" + String(maxRetries) + ")");
        ...
        // 成功
        Serial.println("✅ ID " + String(id) + " 恢复成功" + (retry > 0 ? " (重试" + String(retry) + "次后成功)" : ""));
        return true;
    }
 ...
 * 恢复所有指纹（增强版：带失败列表）
 */
int restoreAllFingerprints(ProgressCallback progressCallback = nullptr) {
    Serial.println("==================== 开始恢复所有指纹 ====================");
    Serial.println("📊 待恢复数量: " + String(backupCount));
    
    if (fingerprintBackup == nullptr || backupCount == 0) {
        Serial.println("❌ 无备份数据");
        return 0;
    }
    
    int successCount = 0;
    int failedCount = 0;
    String failedIds = "";  // 记录失败的ID
    unsigned long startTime = millis();
    
    for (int i = 0; i < backupCount; i++) {
        int id = fingerprintBackup[i].id;
        
        // 更新进度
        if (progressCallback != nullptr) {
            int percent = (i * 100) / backupCount;
            String status = "恢复中: " + String(successCount) + "/" + String(backupCount) + 
                          " (失败:" + String(failedCount) + ")";
            progressCallback(percent, 100, status);
        }
        
        if (restoreSingleFingerprint(i)) {
            successCount++;
        } else {
            failedCount++;
            ...
            Serial.println("⚠️  ID " + String(id) + " 最终失败 (已重试3次)");
        }
    ...
    Serial.println("==================== 恢复完成 ====================");
    Serial.println("✅ 成功恢复: " + String(successCount) + "/" + String(backupCount));
    Serial.println("❌ 失败数量: " + String(failedCount));
    
    if (failedCount > 0) {
        Serial.println("❌ 失败ID列表: " + failedIds);
        Serial.println("");
        Serial.println("💡 建议:");
        Serial.println("   1. 检查新传感器连接是否稳定");
        Serial.println("   2. 重新执行备份和恢复流程");
        Serial.println("   3. 如果特定ID总是失败，可能是该指纹数据损坏");
    }
    
    Serial.println("⏱️  总耗时: " + String(duration) + " 秒");
    Serial.println("📊 成功率: " + String(successCount * 100.0 / backupCount, 1) + "%");
    Serial.println("=================================================");
...
String getBackupStats() {
    ...
    String stats = "";
    stats += "备份数量: " + String(backupCount) + "\n";
    stats += "内存占用: " + String((backupCount * sizeof(FingerprintTemplate)) / 1024.0, 2) + " KB\n";
    stats += "使用率: " + String(backupCount * 100.0 / 1000, 1) + "%";
 ...
 * 打印详细统计
 */
void printBackupStats() {
    Serial.println("==================== 备份统计 ====================");
    Serial.println(getBackupStats());
    
    // 显示部分ID列表
    if (backupCount > 0) {
        Serial.print("ID列表: ");
        int showCount = min(backupCount, 20);
        for (int i = 0; i < showCount; i++) {
            Serial.print(String(fingerprintBackup[i].id));
            if (i < showCount - 1) Serial.print(", ");
        }
        if (backupCount > 20) {
            Serial.print(" ... (共" + String(backupCount) + "个)");
        }
        Serial.println();
    }
    Serial.println("=================================================");
 ...
 * 创建迁移进度界面
 */
void createMigrationProgressScreen(String title) {
    Serial.println("创建迁移进度界面: " + title);
    
    // 清理旧界面
    if (migrationProgressScreen != NULL) {
        lv_obj_del(migrationProgressScreen);
    }
    
    // 创建全屏界面
    migrationProgressScreen = lv_obj_create(NULL);
    lv_obj_set_size(migrationProgressScreen, LV_HOR_RES, LV_VER_RES);
    lv_obj_set_style_bg_color(migrationProgressScreen, lv_color_hex(0xF0F8FF), 0);
    
    // 标题
    migrationTitleLabel = lv_label_create(migrationProgressScreen);
    lv_label_set_text(migrationTitleLabel, title.c_str());
    lv_obj_set_style_text_font(migrationTitleLabel, &myFont_new, 0);
    lv_obj_set_style_text_color(migrationTitleLabel, lv_color_hex(0x2196F3), 0);
    lv_obj_align(migrationTitleLabel, LV_ALIGN_TOP_MID, 0, 30);
    
    // 进度条
    migrationBar = lv_bar_create(migrationProgressScreen);
    lv_obj_set_size(migrationBar, 280, 20);
    lv_obj_align(migrationBar, LV_ALIGN_CENTER, 0, -20);
    lv_bar_set_range(migrationBar, 0, 100);
    lv_bar_set_value(migrationBar, 0, LV_ANIM_OFF);
    
    // 进度百分比标签
    migrationProgressLabel = lv_label_create(migrationProgressScreen);
    lv_label_set_text(migrationProgressLabel, "0%");
    lv_obj_set_style_text_font(migrationProgressLabel, &myFont_new, 0);
    lv_obj_align(migrationProgressLabel, LV_ALIGN_CENTER, 0, 10);
    
    // 状态标签
    migrationStatusLabel = lv_label_create(migrationProgressScreen);
    lv_label_set_text(migrationStatusLabel, "准备中...");
    lv_obj_set_style_text_font(migrationStatusLabel, &myFont_new, 0);
    lv_obj_set_style_text_color(migrationStatusLabel, lv_color_hex(0x666666), 0);
    lv_obj_set_style_text_align(migrationStatusLabel, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_align(migrationStatusLabel, LV_ALIGN_CENTER, 0, 40);
    lv_obj_set_width(migrationStatusLabel, 280);
    
    // 提示信息
    lv_obj_t *hintLabel = lv_label_create(migrationProgressScreen);
    lv_label_set_text(hintLabel, "请勿断电或触摸屏幕");
    lv_obj_set_style_text_font(hintLabel, &myFont_new, 0);
    lv_obj_set_style_text_color(hintLabel, lv_color_hex(0xFF9800), 0);
    lv_obj_align(hintLabel, LV_ALIGN_BOTTOM_MID, 0, -30);
    
    // 显示界面
    lv_scr_load(migrationProgressScreen);
}

/**
 * 更新迁移进度
 */
void updateMigrationProgress(int current, int total, String status) {
    if (migrationProgressScreen == NULL) return;
    ...
    // 更新进度条
    if (migrationBar != NULL) {
        lv_bar_set_value(migrationBar, percent, LV_ANIM_OFF);
    }
    
    // 更新百分比文字
    if (migrationProgressLabel != NULL) {
        lv_label_set_text(migrationProgressLabel, (String(percent) + "%").c_str());
    }
    
    // 更新状态文字
    if (migrationStatusLabel != NULL) {
        lv_label_set_text(migrationStatusLabel, status.c_str());
    }
 ...
 * 关闭迁移进度界面
 */
void closeMigrationProgressScreen() {
    if (migrationProgressScreen != NULL) {
        lv_scr_load(mainScreen);
        lv_obj_del(migrationProgressScreen);
        migrationProgressScreen = NULL;
        migrationTitleLabel = NULL;
        migrationProgressLabel = NULL;
        migrationStatusLabel = NULL;
        migrationBar = NULL;
    }
}

// 备份按钮事件
void btnBackup_event_cb(lv_event_t * e) {
    Serial.println("==================== 备份按钮被点击 ====================");
    
    if (!fingerprintSystemReady) {
        showMessageBox("系统错误", "指纹传感器未就绪", "确定", false);
        return;
    }
    ...
    if (freePsram < 200000) {
        showMessageBox("内存不足", "PSRAM可用空间不足\n请重启设备", "确定", false);
        return;
    }
    ...
    msg += "3. 备份所有数据\n\n";
    msg += "预计 3-10 分钟";
    showMessageBox("开始备份", msg, "确认", true);
    ...
    lv_timer_create([](lv_timer_t * timer) {
        ...
        // 显示进度界面
        createMigrationProgressScreen("智能备份");
        
        // 执行智能备份
        int count = backupAllFingerprints([](int current, int total, String status) {
            updateMigrationProgress(current, total, status);
        });
        
        // 关闭进度界面
        closeMigrationProgressScreen();
        ...
        if (count > 0) {
            ...
            msg += getBackupStats() + "\n\n";
            msg += "请更换传感器后\n点击\"恢复\"按钮";
            showMessageBox("备份完成", msg, "确定", true);
            
            printBackupStats();
        } else {
            showMessageBox("备份失败", "未发现任何指纹\n请检查源传感器", "确定", false);
        }
...
// 恢复按钮事件
void btnRestore_event_cb(lv_event_t * e) {
    Serial.println("==================== 恢复按钮被点击 ====================");
    ...
    if (fingerprintBackup == nullptr || backupCount == 0) {
        showMessageBox("无备份数据", "请先执行\"备份\"操作", "确定", false);
        return;
    }
    ...
    msg += getBackupStats() + "\n\n";
    msg += "⚠️ 此操作不可逆";
    showMessageBox("开始恢复", msg, "确认", true);
    ...
    lv_timer_create([](lv_timer_t * timer) {
        ...
        // 显示进度界面
        createMigrationProgressScreen("恢复指纹");
        
        // 执行恢复
        int count = restoreAllFingerprints([](int current, int total, String status) {
            updateMigrationProgress(current, total, status);
        });
        ...
        if (count > 0) {
            String msg = "✅ 恢复成功\n\n";
            msg += "成功: " + String(count) + " 个\n";
            msg += "失败: " + String(backupCount - count) + " 个\n\n";
            msg += "迁移完成！";
            showMessageBox("恢复完成", msg, "确定", true);
        } else {
            showMessageBox("恢复失败", "写入失败\n请检查目标传感器", "确定", false);
        }
